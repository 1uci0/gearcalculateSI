...
function AnaliseGeometrica() {
    ...
    if (psi != 0) {
        if (modulo === 'mt' ) {
            var m_t = Number(window.document.getElementById('m-t').value); //ok
            var phi_t = Number(window.document.getElementById('phi-t').value); //ok

            var m_n = m_t*Math.cos(psiRadianos); //ok
            ...
            var m_n = Number(window.document.getElementById('m-n').value); //ok
            var phi_n = Number(window.document.getElementById('phi-n').value); //ok

            var m_t = m_n/Math.cos(psiRadianos); //ok
    ...
    if (varPreenchida < 2) {
        ...
        document.getElementById('mn-mt').disabled = true; //ok
        document.getElementById('mt-mn').disabled = true; //ok

        document.getElementById('m-t').disabled = true; //ok
        document.getElementById('m-n').disabled = true; //ok
        ...
        for (let i = 0; i < 2; i++) {
            dv[i] = d[i] + 2*m_n*x[i]; //ok
            de[i] = d[i] + 2*m_n*(ka + x[i]); //ok
        ...
        for (let i = 0; i < 2; i++) {
            if (i == 0) {
                dr[i] = 2*(C - 0.5*de[i+1] - 0.25*m_n); //ok
            }
            else if (i == 1) {
                dr[i] = 2*(C - 0.5*de[i-1] - 0.25*m_n); //ok
            }
            ...
            st[i] = (m_n)*(0.5*Math.PI + 2*x[i]*Math.tan(phi_n_Radianos)); //ok
        }
        ...
        window.document.getElementById('M-n').value = m_n.toFixed(2); //ok
        window.document.getElementById('Phi-n-n').value = phi_n.toFixed(2); //ok
        window.document.getElementById('M-t').value = m_t.toFixed(2); //ok
        ...
        localStorage.m_n = m_n; //ok 
        localStorage.m_t = m_t; //ok 
...
function AnaliseCinematicaDinamica() {
    ...
    if (unidadeH === 'W') {
        var H = Number(window.document.getElementById('H').value)/1e3; //ok
        
        if (H > 1.35962e-3*25e3) {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
    ...
    if (varPreenchida < 2) {
        ...
        if (H == '') {
            ...
            if (n1 != '') {
                ...
                var wt = 2*t2/d2;  //ok
                var n2 = 6e4*H/(Math.PI*d2*wt);  //ok

                var n1 = i*n2;  //ok
                var t1 = t2/i;  //ok
            }
        }
        var H = (Math.PI*d1*n1*wt)/6e4;  //ok
        ...
        window.document.getElementById('w').value = w.toFixed(3); //ok
        window.document.getElementById('w-t').value = wt.toFixed(3); //ok
        window.document.getElementById('w-r').value = wr.toFixed(3); //ok
        window.document.getElementById('w-a').value = wa.toFixed(3); //ok
...
function AnaliseTensoesFlexao() {
    ...
    if (varPreenchida < 6) {
        ...
        // buscando do localStorage
        var wt = 1e3*Number(localStorage.wt); //ok
        ...
        var i = Number(localStorage.i); //ok
        var m_t = Number(localStorage.m_t); //ok
        var m_n = Number(localStorage.m_n); //ok
        var phi_n_Radianos = Number(localStorage.phi_n_Radianos); //ok
        var psiRadianos = Number(localStorage.psiRadianos); //ok
        var phi_t_Radianos = Number(localStorage.phi_t_Radianos); //ok
        ...
        var Vt_max = [A + (Qv - 3)]**2/196.85; //ok
        var v = [v1, v2]; //ok
        var kv = []; //ok

        for (let i = 0; i < 2; i++) {
            if (v[i] < Vt_max) {
                kv[i] = ((A + Math.sqrt(196.85*v[i]))/A)**B; //ok
            }
            else {
            ...
            // km = f(Cmf, Cmt) - Cmf > fator de distribuição de carga de face | Cmt > fator de distribuição de carga transversal >>> km = Cmf - Método empírico
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    if ((F[i]/d[i]) <= 2 && F[i] <= 1020) {
                        ...
                        else if (F[i] > 25 && F[i] <= 432) {
                            Cpf[i] = F_10d[i] - 0.0375 + 0.000492*F[i]; //ok
                        }
                        else {
                            Cpf[i] = F_10d[i] - 0.1109 + 0.000815*F[i] - 0.000000353*F[i]**2; //ok 
                        }
        ...
        var deixo = [deixo1, deixo2]; // VOLTAR AQ E TROCAR O VALOR
        ...
        // Pinion
        var R1 = z1*mn/(2*Math.cos(ψ)); 
        var Ro1 = R1 + mn*(ka + x1); 

        // Gear
        var x2 = -x1;
        var R2 = z2*mn/(2*Math.cos(ψ)); 
        var Ro2 = R2 + mn*(ka + x2); 
        ...
        for (let i = 0; i < 2; i++) {
            σf[i] = wt*ko[i]*kv[i]*ks[i]*(1/(m_t*Math.min(F1,F2)))*(km[i]*kb[i]/J[i]);
...
function AnaliseTensoesContato() {
    ...
    if (varPreenchida < 5) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            window.document.getElementById('ch-'+(i+1)).value = CH[i].toFixed(1); //ok
        }
...
function HB1() {
    ...
    if (materialPinhao == 1) {
        ...
        var St1 = 0.533*mySlider.value + 88.3;
        var Sc1 = 2.22*mySlider.value + 200;
    }
        ...
        var St1 = 0.703*mySlider.value + 113;
        var Sc1 = 2.41*mySlider.value + 237;
    }
        ...
        var St1 = 0.568*mySlider.value + 83.8;
        var Sc1 = 1035;
    }
        ...
        var St1 = 0.749*mySlider.value + 110;
        var Sc1 = 1125;
    }
        ...
        var St1 = 0.594*mySlider.value + 87.76;
        var Sc1 = 1170;
    }
        ...
        var St1 = 0.7848*mySlider.value + 114.81;
        var Sc1 = 1260;
    }
        ...
        var St1 = 0.7255*mySlider.value + 63.98;
        var Sc1 = 1070;
    }
        ...
        var St1 = 0.7255*mySlider.value + 153.63;
        var Sc1 = 1185;
    }
        ...
        var St1 = 0.7255*mySlider.value + 201.91;
        var Sc1 = 1305;
    }
...
function HB2() {
    ...
    if (materialCoroa == 1) {
        ...
        var St2 = 0.533*mySlider.value + 88.3;
        var Sc2 = 2.22*mySlider.value + 200;
    }
        ...
        var St2 = 0.703*mySlider.value + 113;
        var Sc2 = 2.41*mySlider.value + 237;
    }
        ...
        var St2 = 0.568*mySlider.value + 83.8;
        var Sc2 = 1035;
    }
        ...
        var St2 = 0.749*mySlider.value + 110;
        var Sc2 = 1125;
    }
        ...
        var St2 = 0.594*mySlider.value + 87.76;
        var Sc2 = 1170;
    }
        ...
        var St2 = 0.7848*mySlider.value + 114.81;
        var Sc2 = 1260;
    }
        ...
        var St2 = 0.7255*mySlider.value + 63.98;
        var Sc2 = 1070;
    }
        ...
        var St2 = 0.7255*mySlider.value + 153.63;
        var Sc2 = 1185;
    }
        ...
        var St2 = 0.7255*mySlider.value + 201.91;
        var Sc2 = 1305;
    }
